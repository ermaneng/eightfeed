...
function getItems()
{
    ...
    {
        $sql = sprintf("SELECT fs.feed_name, i.title, i.link, i.pubdate, i.guid, i.description, i.summary, i.imgWidth, i.imgHeight, i.imgsrc, i.feed_hashtag FROM items_tech AS i, feed_sources_tech AS fs WHERE fs.hashtag = i.feed_hashtag AND i.feed_hashtag = '%s' ORDER BY pubdate DESC LIMIT 0, %d",$value,$limitForEachSource);
        $query = mysql_query($sql,$connection);
        $total = mysql_num_rows($query) ;

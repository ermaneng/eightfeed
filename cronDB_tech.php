...
function getFeedSources(){
    global $connection;
    //get sources
    $sql = sprintf("SELECT feed_name,hashtag,url FROM feed_sources_tech WHERE active='1'");
...
function saveItems($items)
{
    ...
    for ($i=0; $i<count($items); $i++)
    {
        ...
        $sql = sprintf("INSERT INTO items_tech (item_id, title, link, pubdate, guid, description, summary, imgWidth, imgHeight, imgsrc, feed_hashtag) VALUES (%d, '%s', '%s', '%s', '%s', '%s', '%s', %d, %d, '%s', '%s'); ",
